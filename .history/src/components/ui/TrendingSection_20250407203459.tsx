  ...
  return (
    <section className="container mx-auto px-4 mb-8">
      <div className="flex flex-col md:flex-row gap-6">
        {/* Trending Coins Card */}
        <div className="w-full md:w-1/2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md p-4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">Trending Coins</h2>
            <a href="#" className="text-blue-600 dark:text-blue-400 text-sm hover:underline">More &gt;</a>
          </div>

          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {trendingCoins.map((coin, index) => (
              <div key={coin.id} className="py-3 flex items-center">
                <div className="w-6 text-gray-500 dark:text-gray-400 text-sm">{index + 1}</div>
                ...
                <div className="flex-1">
                  <div className="flex items-center">
                    ...
                    <span className="text-gray-500 dark:text-gray-400 text-sm ml-2">{coin.symbol}</span>
                  </div>
                </div>
                <div className="flex flex-col items-end">
                  <span className="font-medium text-gray-900 dark:text-white">{formatCurrencyWithDecimals(coin.price)}</span>
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {trendingDexPairs.map((pair, index) => (
              <div key={pair.id} className="py-3 flex items-center">
                ...
                <div className="flex-1">
                  <div className="flex items-center">
                    <span className="font-medium text-sm text-gray-900 dark:text-white">{pair.name}</span>
                  </div>
                </div>
                <div className="flex flex-col items-end">
                  <span className="font-medium text-gray-900 dark:text-white">{formatCurrencyWithDecimals(pair.price)}</span>
